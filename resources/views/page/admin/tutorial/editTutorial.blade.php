...
<script>
    ...
    // /tutorialDetail/{id}
    console.log('/tutorialDetail/'+idTutorial);
    $(document).ready(function(){
        $.ajax({
            ...
            success: function(response){
                ...
                $('#inputBahan').val(response.data.bahan);
                $('#inputAlat').val(response.data.alat);
                $('#inputLangkah').val(response.data.langkah_tutorial);
                $('#prevImg').attr("src", `{{ asset('storage/assets/fotos/${response.data.foto}')}}`);
                $('form').attr('action', '/dashboard/admin/tutorial/'+idTutorial+'/edit');
            },
